...
with open(csv_file, 'w', newline='') as file:
    ...
    writer.writerow(['frame_path', 'steering_angle', 'direction'])  # 헤더 작성
    ...
    while cap.isOpened():
        ...
        if collecting_data and (current_time - last_saved_time >= frame_interval):
            ...
            with open(csv_file, 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([frame_path, current_angle, current_direction])

            print(f"Saved: {frame_path}, Steering Angle: {current_angle}, Direction: {current_direction}")
            frame_count += 1
        ...
        cv2.putText(display_image, f"Direction: {current_direction}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
